...
public class CheckinManageAction extends ActionSupport {
	...
	/**
	 * 确定签到时间{@code now}对应哪个值班时间段，范围为0~4，对应5个值班时间段； 返回-1表示没有对应的值班时间段
	 */
	...
	/**
	 * 检查{@code now}是否可以签到. 返回值分为标识码和相应信息 标志码为0表示可以签到，其余表示不可以签到
	 */
	static String checkCanCheckin(Session s, java.util.Date now) {
		...
		List checkinRecordList = s.createCriteria(CheckInRecord.class).add(Restrictions.eq("student.id", student_id))
		...
		if (currentPeriodId == -1) {
			...
			if (status.charAt(0) == '0') {
				...
				List ds_list = (List) s.createCriteria(model.DutySchedule.class)
						.add(Restrictions.eq("student.id", student_id)).createAlias("dutyPiece", "dutyPiece")
						.add(Restrictions.eq("dutyPiece.time", cntTime)).list();
				// 当前学生选择了此时间段的选班，可以进行签到
				if (ds_list.size() > 0) {
					status = "0对于值班时间段" + util.Util.dutyPeriodList.get(currentPeriodId) + "进行签到";
				} else {
					// 当前学生没有选择此时间段的选班，不可以进行签到
					status = "3当前学生没有选择" + util.Util.dutyPeriodList.get(currentPeriodId) + "时间段的选班，不可以进行签到";
				}
			}
	...
	public String execute() throws Exception {
		...
		checkinRecordList = s.createCriteria(CheckInRecord.class).add(Restrictions.eq("student.id", student_id))
				.addOrder(Order.desc("id")).list();
		s.close();
	...
	//
	// checkinRecordTable =
	// Util.getJspOutput("/jsp/admin/widgets/checkinRecordTable.jsp");
	// session.close();
	// return ActionSupport.SUCCESS;
	// }
	//
	...
	// row.createCell(0,
	// Cell.CELL_TYPE_STRING).setCellValue(cr.getStudent().getUser().getFullName());
	// row.createCell(1,
	// Cell.CELL_TYPE_STRING).setCellValue(cr.getStudent().getStudentId());
	// row.createCell(2,
	// Cell.CELL_TYPE_STRING).setCellValue(util.Util.formatTimestamp(cr.recordtime));
	// }
	//
	...
	// {
	//
	// FileOutputStream fout = new FileOutputStream(util.Util.RootPath +
	// checkinRecordExcelPath);
	// wb.write(fout);
	// fout.close();
	// wb.close();
	// }
