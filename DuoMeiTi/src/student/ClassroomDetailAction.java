...
// String repertory_hql="select r from Repertory r where
// r.classroom="+classroom_id;
...
//// int checkrecord_start = ((int) check_rowCount) > 5 ? ((int) check_rowCount)
// - 5 : 0;
...
//
// long repair_rowCount = (Long) session.createQuery("select count(*) from
// RepairRecord as rd left join rd.device as ry left join ry.classroom as cm
// where cm.id=" + classroom_id).uniqueResult();
//// int repairrecord_start = ((int) repair_rowCount) > 5 ? ((int)
// repair_rowCount) - 5 : 0;
...
//
// classroom_repertory_list =
// session.createCriteria(model.Repertory.class).add(Restrictions.eq("classroom.id",
// classroom_id)).list();
//
