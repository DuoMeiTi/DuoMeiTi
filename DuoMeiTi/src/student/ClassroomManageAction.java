...
public class ClassroomManageAction extends ActionSupport {
	...
	public String classroomList() throws Exception {
		...
		classrooms = s.createCriteria(model.Classroom.class).add(Restrictions.eq("principal.id", student_id)).list();
		// String hql = "SELECT c.id,c.classroom_num,u.id,u.fullName,t.id from
		// Classroom c,User u,TeachBuilding t,StudentProfile s where
		// c.teachbuilding=t and c.principal=s and s.user=u and u.id="+user_id;
		//
