...
public class DutyManageAction {
	...
	public String obtainDutyTable() throws Exception {
		...
		List<model.DutySchedule> L = (List<model.DutySchedule>) session.createCriteria(model.DutySchedule.class)
				.add(Restrictions.eq("student.id", studentId)).list();
	...
	public synchronized String deleteSchedule() throws Exception {
		synchronized (this.getClass()) {
			...
			model.DutySchedule ds = (DutySchedule) session.createCriteria(model.DutySchedule.class)
					...
					.add(Restrictions.eq("student.id", deleteSchedule_studentId)).uniqueResult();
	...
	public synchronized String addSchedule() throws Exception {
		synchronized (this.getClass()) {
			...
			model.DutyPiece dp = (DutyPiece) session.createCriteria(model.DutyPiece.class)
					.add(Restrictions.eq("id", addSchedule_dutyPieceId)).uniqueResult();

			model.StudentProfile sp = (model.StudentProfile) session.createCriteria(model.StudentProfile.class)
					.add(Restrictions.eq("id", addSchedule_studentId)).uniqueResult();

			model.DutySchedule ds = (DutySchedule) session.createCriteria(model.DutySchedule.class)
					.add(Restrictions.eq("dutyPiece.id", addSchedule_dutyPieceId))
					.add(Restrictions.eq("student.id", addSchedule_studentId)).uniqueResult();
	...
	// Session session = model.Util.sessionFactory.openSession();
	// String hql="from DutyTime d where
	// d.teachBuilding.build_id="+teachBuildingId;
	//
	// String studentSelect="from StudentProfile s where s.id="+studentId;
	// StudentProfile
	// s=(StudentProfile)session.createQuery(studentSelect).list().get(0);
	//
	// String selectChosen="select ds.dutyTime.time from DutySchedule ds where
	// ds.student.id="+s.id+" and "
	...
	// ChooseClassSwitch
	// f=(ChooseClassSwitch)session.createQuery(hql).list().get(0);
	// if(!f.open){
	// log="closed";
	// session.close();
	// return "ajaxSuccess";
	...
	// String dutySelect="select ds.dutyTime from DutySchedule ds where
	// ds.student.id="
	...
	// String back= "update DutyTime d set d.dutyLeft=d.dutyLeft+1 where
	// d.id="+tmp.id;
	// session.createQuery(back).executeUpdate();
	// }
	// String selectDutySchedule="From DutySchedule ds where
	// ds.student.id="+studentId+" and "
	// +"ds.dutyTime.teachBuilding.build_id="+teachBuildingId;
	// List<DutySchedule>
	// dutySchedules=session.createQuery(selectDutySchedule).list();
	...
	// //dutyTime数据库更新
	// trans=session.beginTransaction();
	// String updateDutyLeft="update DutyTime d set d.dutyLeft=d.dutyLeft-1
	// where d.teachBuilding.build_id="
