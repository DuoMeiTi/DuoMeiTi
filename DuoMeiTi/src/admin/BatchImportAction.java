...
public class BatchImportAction extends FileUploadBaseAction {
	...
	// classroom.getClass_schedule_path());
	...
	// }
	//
	// util.Util.saveFile(file, fileFileName, util.Util.RootPath +
	...
	public String classroomPrincipalShow() {
		...
		if (selectTeachBuilding > 0) {
			classroom_list = s.createCriteria(Classroom.class)
					.add(Restrictions.eq("teachbuilding.id", selectTeachBuilding)).addOrder(Order.asc("classroom_num"))
					.list();
	...
	// if(selectTeachBuilding != -1)
	// {
	// classroom_criteria.add(Restrictions.eq("teachbuilding.id",
	// selectTeachBuilding));
	// classroom_criteria.addOrder(Order.asc("classroom_num"));
	// classroom_list = classroom_criteria.list();
	//
	// }
	// classroomcheckbox =
	// util.Util.getJspOutput("/jsp/admin/widgets/classroomPrincipalCheckbox.jsp");
	// session.close();
	//
	// return SUCCESS;
	// }
		...
		try {
			...
			if (classroomIdList == null || classroomIdList.size() == 0) {
				...
				for (int i = 0; i < classroomIdList.size(); ++i) {
					...
					Classroom cnt = (Classroom) s.createCriteria(model.Classroom.class)
							.add(Restrictions.eq("id", classroom_id)).uniqueResult();
