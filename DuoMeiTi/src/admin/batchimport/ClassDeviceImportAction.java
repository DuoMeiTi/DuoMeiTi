...
public class ClassDeviceImportAction extends util.FileUploadBaseAction {
	...
	public String execute() throws Exception {
		...
		List<Repertory> deviceList =
				...
				util.Util.getListWithOneEqualRestriction(s, Repertory.class, "rtClassroom.teachbuilding.build_id",
						execute_selectTeachBuilding);
	...
	private static Classroom obtainClassoom(Session session, String teachBuildingName, Cell classroomCell) {
		...
		Criteria classroomCriteria = util.Util.getCriteriaWithOneEqualRestriction(session, model.Classroom.class,
				"teachbuilding.build_name", teachBuildingName);
	...
	// 返回值表示对应的当前的 教室负责人
	private static StudentProfile saveClassroomPrincipal(Session session, Classroom classroom, String principalFullName,
			StudentProfile previousPrincipal) {
		StudentProfile principal = null;
		if (principalFullName.isEmpty()) {
			principal = previousPrincipal;
		} else {
			List<StudentProfile> principals = util.Util.getListWithOneEqualRestriction(session,
					model.StudentProfile.class, "user.fullName", principalFullName);
	...
	// 表示是否存储成功
	private static boolean saveClassroomDevice(Session session, Classroom classroom, String deviceName,
			String deviceInfo) {
		...
		Criteria deviceCriteria = util.Util.getCriteriaWithOneEqualRestriction(session, model.Repertory.class,
				"rtClassroom.id", classroom.id);
			...
			for (int sheetId = 0; sheetId < workbook.getNumberOfSheets(); ++sheetId) {
				...
				for (int rowId = 1;; rowId++) {
					...
					for (int colId = 1; colId < 20; colId++) {
						if (firstRow.getCell(colId) == null)
							continue;
						...
						if (firstRowCellValue.equals("负责人")) {
							previousPrincipal = saveClassroomPrincipal(session, classroom, cellValue,
									previousPrincipal);
						}
	...
	// }
	//
	// public void setExecute_classroomList(List<Classroom>
