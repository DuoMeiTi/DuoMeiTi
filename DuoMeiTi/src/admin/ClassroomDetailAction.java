...
public class ClassroomDetailAction extends FileUploadBaseAction {
	...
	public void ClassroomPicture() {
		...
		Session session = model.Util.sessionFactory.openSession();
		Criteria c1 = session.createCriteria(RoomPicture.class).add(Restrictions.eq("class_id", classroomId)); // hibernate
																												// session创建查询
		picture_list = c1.list();

		Criteria q = session.createCriteria(Classroom.class).add(Restrictions.eq("id", classroomId)); // hibernate
																										// session创建查询
	...
	public String PictureDelete() {
		...
		// 从数据库中删除
		Session session = model.Util.sessionFactory.openSession();
		Criteria q = session.createCriteria(RoomPicture.class).add(Restrictions.eq("id", picID)); // hibernate
																									// session创建查询
		picture_list = q.list();
	...
	public String ScheduleUpload() {
		...
		// Criteria q =
		// session.createCriteria(Classroom.class).add(Restrictions.eq("id",classroomId));
		...
		// if (file != null)//file没接收到的原因可能是jsp页面里面的input file的属性名不是file
		// {
		// File old_class_schedule = new File(util.Util.RootPath +
		// nClass.getClass_schedule_path());
		...
		// String newFileName = nClass.getTeachbuilding().getBuild_name() + "-"
		// + nClass.getClassroom_num()
		// + fileFileName.substring(fileFileName.lastIndexOf("."));
		//
		// util.Util.saveFile(file, newFileName, util.Util.RootPath +
		// util.Util.ClassroomSchedulePath);
		// String inserted_file_path = util.Util.ClassroomSchedulePath
		// +newFileName;
