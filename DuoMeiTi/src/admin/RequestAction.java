...
public class RequestAction {
	 ...
	 * 
	 * public void setUser_list(List<User> user_list) { this.user_list =
	 * user_list; }
	 */
	...
	public String studentRequestEnsure() throws Exception {
		...
		String hql = "update StudentProfile s set s.isPassed=" + String.valueOf(isPassed) + " where s.id="
	...
	public String studentRequestChange() throws Exception {
		Session session = model.Util.sessionFactory.openSession();
		Transaction trans = session.beginTransaction();
		String hql = "update StudentProfile s set s.isPassed=0" + " where s.id=" + String.valueOf(userid);
		Query q = session.createQuery(hql);
		int ret = q.executeUpdate();
		System.out.println(ret);
		trans.commit();
