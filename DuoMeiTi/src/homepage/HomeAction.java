...
public class HomeAction extends PageGetBaseAction {
	...
	static {
		...
		List L = s.createCriteria(model.SemesterFirstWeek.class).list();
		if (L.size() == 0) {
			s.beginTransaction();
	...
	public String execute() throws Exception {
		...
		// 确定当前值班同学：
		ArrayList<model.DutySchedule> allDutyList = (ArrayList<model.DutySchedule>) session
				.createCriteria(model.DutySchedule.class).list();
		...
		// HomepageInformation.obtainLastWeekNotCheckClassroomStudentList(session);

		allNotCheckStudentList = HomepageInformation.obtainLastWeekNotCheckClassroomStudentList(session);

		// notCheckClassroomStudentList =
		// notCheckClassroomStudentList.subList(0,
		// Math.min(notCheckClassroomStudentList.size(), MaxRes));
	...
	public String resourceFile() throws Exception {
		...
		// file_path_list = this.makeCurrentPageList(q, 10); // 根据代表总体的Criteria
		// 获取当前页元素的List，这个效率高，应尽量使用这个
		// file_path_list = this.makeCurrentPageList(q.list(), 10);
		// //根据代表总体的List 获取当前页元素的List
		...
		//// file_path_html =
		// util.Util.getJspOutput("/jsp/admin/HomepageModify/ResourceFileTable.jsp");
		// return "getPage";
		// }
	...
	public String dutyShow() throws Exception {
		...
		for (int i = 0; i < dutyScheduleList.size(); ++i) {
			...
			dutyShowList.get(dutyPlaceList.indexOf(ds.dutyPiece.dutyPlace)).get(ds.dutyPiece.time).add(ds);
	...
	public String classroomPincipalShow() throws Exception {
		...
		Session s = model.Util.sessionFactory.openSession();
		List<Classroom> allClassroomList = s.createCriteria(model.Classroom.class).addOrder(Order.asc("classroom_num"))
				.list();
