...
public class HomepageInformation extends util.PageGetBaseAction {
	...
	static boolean isEmptyInCheckRecordList(List<model.CheckRecord> L, int stu_user_id, int classroom_id,
		...
		for (model.CheckRecord cr : L) {
			if (cr.checkman.id == stu_user_id && cr.classroom.id == classroom_id && cr.checkdate.after(beg)
	...
	static ArrayList<ArrayList<StudentProfile>> obtainLastWeekNotCheckClassroomStudentList(Session s) {
		ArrayList<ArrayList<StudentProfile>> allNotCheckStudentList = new ArrayList<ArrayList<StudentProfile>>();

		model.SemesterFirstWeek firstWeek = (model.SemesterFirstWeek) s.createCriteria(model.SemesterFirstWeek.class)
				.uniqueResult();

		if (firstWeek.date == null)
		...
		for (;;) {
			...
			mondayList.add(cnt);
			if (isSameDay(cnt, nowMonday))
				break;
			cal.add(Calendar.DATE, 7);
		}

		ArrayList<Classroom> all_classroom = (ArrayList<Classroom>) s.createCriteria(model.Classroom.class)
				.add(Restrictions.isNotNull("principal")).list();

		// 得到所有大于起始周的所有检查记录
		ArrayList<model.CheckRecord> checkRecordList = (ArrayList<model.CheckRecord>) s
				.createCriteria(model.CheckRecord.class).add(Restrictions.ge("checkdate", firstWeekMonday)).list();
		...
		for (int i = 0; i < mondayList.size() - 1; ++i) {
			...
			for (Classroom c : all_classroom) {
				...
				// .add(Restrictions.eq("checkman.id", c.principal.user.id))
				// .add(Restrictions.eq("classroom.id", c.id))
				// .add(Restrictions.between("checkdate", mondayList.get(i),
				// mondayList.get(i + 1)))
				// .list().isEmpty();

				boolean empty = isEmptyInCheckRecordList(checkRecordList, c.principal.user.id, c.id, mondayList.get(i),
						mondayList.get(i + 1));
	...
	static Criteria obtainAllNoticeCriteria(Session s) {
		return s.createCriteria(model.Notice.class).addOrder(Order.desc("id"));
	}
	...
	static Criteria obtainAllReplaceDeviceCriteria(Session s) {
		...
		Criteria q = s.createCriteria(model.Repertory.class).add(Restrictions.le("rtDeadlineDate", sql_now))
				.add(Restrictions.eq("rtDeviceStatus", util.Util.DeviceClassroomStatus))
				.add(Restrictions.eq("rtType", "灯泡")).addOrder(Order.desc("id"));
		return q;
	}
	...
	public String MoreAnnouncement() throws Exception {
		...
		// {
		// notice_list_html =
		// util.Util.getJspOutput("/jsp/homepage/widgets/more_announcementTable.jsp");
		// return "getPage";
		// }
	...
	public String MoreCheckClassroom() throws Exception {
		...
		// Criteria q =
		// session.createCriteria(model.CheckRecord.class).addOrder(Order.desc("id"));
		...
		// {
		// check_list_html
		// =util.Util.getJspOutput("/jsp/homepage/widgets/more_classroomTable.jsp");
		// return "getPage";
		// }
		return ActionSupport.SUCCESS;
	}
	...
	public String MoreEquipmentMaintenance() throws Exception {
		...
		// {
		// repair_list_html =
		// util.Util.getJspOutput("/jsp/homepage/widgets/more_equipment_maintenanceTable.jsp");
		// return "getPage";
		// }
		return ActionSupport.SUCCESS;
	}
	...
	public String MoreEquipmentReplacement() throws Exception {
		...
		// .add(Restrictions.eq("rtDeviceStatus",
		...
		// {
		// device_list_html =
		// util.Util.getJspOutput("/jsp/homepage/widgets/more_equipment_replacementTable.jsp");
		// return "getPage";
		// }
	...
	public String notCheckClassroomStudent() throws Exception {
		...
		// obtainLastWeekNotCheckClassroomStudentList(s);
