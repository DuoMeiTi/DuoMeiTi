...
public class RepairRecordManageAction extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		execute_classroomList = util.Util.getCriteriaWithOneEqualRestriction(s, model.Classroom.class,
				"teachbuilding.id", execute_selectTeachBuilding).addOrder(Order.asc("classroom_num")).list();

		if (execute_selectClassroom != -1) {
			Classroom classroom = util.Util.getUniqueResultWithOneEqualRestriction(s, model.Classroom.class, "id",
					execute_selectClassroom);

			execute_devices = util.Util.getListWithOneEqualRestriction(s, model.Repertory.class, "rtClassroom.id",
					execute_selectClassroom);

			Criteria c = util.Util.getCriteriaWithOneEqualRestriction(s, RepairRecord.class, "classroomName",
		...
		// util.Util.getListWithOneEqualRestriction(s, model.Classroom.class,
		// "teachbuilding.id", execute_selectTeachBuilding);
	...
	//
	// Classroom classroom =
	// util.Util.getUniqueResultWithOneEqualRestriction(s,
	// model.Classroom.class, "id", obtain_classroomId);
	//
	// obtain_devices =
	// util.Util.getListWithOneEqualRestriction(s, model.Repertory.class,
	// "rtClassroom.id", obtain_classroomId);
	//
	//
	// System.out.println(obtain_devices.size() + "-----------------------");
	// Criteria c = util.Util.getCriteriaWithOneEqualRestriction(s,
	// RepairRecord.class, "classroomName", classroom.classroom_num);
	// c = util.Util.addOneEqualRestriction(c, "teachingBuildingName",
	// classroom.teachbuilding.build_name);
	// c = c.addOrder(Order.desc("id"));
	// obtain_repairRecords = c.list();
	//
	// obtain_devicesAndrepairRecordsJsp =
	// util.Util.getJspOutput("/jsp/student/widgets/devicesAndRepairRecordsTable.jsp");
	// s.close();
	// return SUCCESS;
	// }
	//
		...
		try {
			...
			for (String studentNameOrId : save_studentNamesOrIds.split(" ")) {
				...
				if (save_studentInfoType.equals("studentNames")) {
					List<StudentProfile> spList = util.Util.getListWithOneEqualRestriction(s,
							model.StudentProfile.class, "user.fullName", studentNameOrId);
					if (spList.size() == 0) {
				...
				{
					StudentProfile sp = util.Util.getUniqueResultWithOneEqualRestriction(s, model.StudentProfile.class,
			...
			Repertory device = util.Util.getUniqueResultWithOneEqualRestriction(s, Repertory.class, "rtId",
					save_deviceId);
			Classroom classroom = util.Util.getUniqueResultWithOneEqualRestriction(s, Classroom.class, "id",
					save_classroomId);
	...
	//
	//
	// public void setObtain_repairRecords(List<RepairRecord>
	// obtain_repairRecords) {
	...
	//
	//
	// public void setObtain_devicesAndrepairRecordsJsp(String
	// obtain_devicesAndrepairRecordsJsp) {
	// this.obtain_devicesAndrepairRecordsJsp =
	// obtain_devicesAndrepairRecordsJsp;
	// }
	//
