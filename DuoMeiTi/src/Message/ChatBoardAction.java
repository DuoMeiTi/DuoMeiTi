...
// String hql1="select u.username,u.id from AdminProfile a,User u where
// a.user=u";
// String hql2="select u.username,u.id from StudentProfile s,User u where
// s.user=u";
...
// newMes.add(new
// MessageInfo(tem.from.username,tem.from.id,tem.to.username,tem.to.id,tem.content));
// }
// session.close();
// }
// catch(Exception e){
...
// messageListHtml =
// util.Util.getJspOutput("/jsp/base/widgets/message_list.jsp", request,
// response);
// return SUCCESS;
// }
//
...
// try{
// Session session = model.Util.sessionFactory.openSession();
// String hql="From Message m where m.from.id="+another+" or m.to.id="+another+"
// order by m.id";
...
// HttpServletResponse response = ServletActionContext.getResponse();
// historyMesHtml =
// util.Util.getJspOutput("/jsp/base/widgets/history_message.jsp", request,
// response);
